        ...
        return $upMethod->body(
            Block::invokeStaticMethod(
                ...
                [
                    ...
                    $this->generateClosure($specification)
                ]
            )
        );
    }

    private function generateClosure(CrudlySpecification $specification): Closure
    {
        return Value::closure(
            [Argument::name('table')->type(new Importable(Blueprint::class))],
            $this->getManager()->getMigrationFieldsDeveloper()->develop($specification)
        );
    }
}
