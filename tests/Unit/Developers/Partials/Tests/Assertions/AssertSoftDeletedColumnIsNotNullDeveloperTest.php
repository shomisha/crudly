...
class AssertSoftDeletedColumnIsNotNullDeveloperTest extends DeveloperTestCase
{
    /**
     * @test
    ...
    public function developer_can_develop_asserting_that_deleted_at_column_is_null(string $modelName, string $modelVar, string $deletedAtColumn)
    {
        ...
        $developer = new AssertSoftDeletedColumnIsNotNull($this->manager, $this->modelSupervisor);
        $block = $developer->develop($specificationBuilder->build(), new CrudlySet());


        $this->assertInstanceOf(InvokeMethodBlock::class, $block);
        $this->assertStringContainsString("\$this->assertNotNull(\${$modelVar}->{$deletedAtColumn});", $block->print());
    }
}
